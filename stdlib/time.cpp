...
class var_time_t : public var_base_t
{
	_time_t m_time;
public:
	var_time_t( const _time_t & time, const int parse_ctr = 0 );
	~var_time_t();
...
#define AS_TIME( x ) static_cast< var_time_t * >( x )

var_time_t::var_time_t( const _time_t & time, const int parse_ctr )
	: var_base_t( VT_CUSTOM, true, parse_ctr ), m_time( time ) {}
var_time_t::~var_time_t() {}

std::string var_time_t::type_str() const { return "time_t"; }
std::string var_time_t::to_str() const
{
	return std::to_string( std::chrono::duration_cast< std::chrono::seconds >( m_time.time_since_epoch() ).count() );
}
mpz_class var_time_t::to_int() const { return ( size_t )std::chrono::duration_cast< std::chrono::seconds >( m_time.time_since_epoch() ).count(); }
bool var_time_t::to_bool() const { return true; }
var_base_t * var_time_t::copy( const int parse_ctr )
{
	return new var_time_t( m_time, parse_ctr );
}
void var_time_t::assn( var_base_t * b )
{
	m_time = AS_TIME( b )->get();
}
_time_t & var_time_t::get() { return m_time; }

var_base_t * time_now( vm_state_t & vm, func_call_data_t & fcd )
{
	return new var_time_t( std::chrono::high_resolution_clock::now() );
}

var_base_t * time_to_hrs( vm_state_t & vm, func_call_data_t & fcd )
{
	_time_t & time = AS_TIME( fcd.args[ 0 ] )->get();
...
{
	_time_t & time1 = AS_TIME( fcd.args[ 1 ] )->get();
	_time_t & time2 = AS_TIME( fcd.args[ 0 ] )->get();
	return new var_time_t( _time_t( time1 - time2 ) );
}
...
REGISTER_MODULE( time )
{
	...
	functions_t & timefns = vm.typefuncs[ "time_t" ];
	...
	vm.funcs.add( { "-", 2,  2, { "time_t", "time_t" }, FnType::MODULE, { .modfn = time_diff }, true } );
}